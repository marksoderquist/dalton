...
public class WeatherUndergroundPublisher extends HttpPublisher {
	...
	public int publish( WeatherStation station, WeatherDataEvent event ) throws IOException {
		return rest( "GET", generatePayload( station, event ) ).getCode();
	}

	public String generatePayload( WeatherStation station, WeatherDataEvent event ) throws IOException {
		// Example:
		...
		long timestamp = event.getValue( WeatherDatumIdentifier.TIMESTAMP );
		...
		// Prepare basic data
		add( event, builder, WeatherDatumIdentifier.TEMPERATURE, "tempf", "0.0" );
		add( event, builder, WeatherDatumIdentifier.PRESSURE, "baromin", "0.00" );
		add( event, builder, WeatherDatumIdentifier.HUMIDITY, "humidity", "0" );
		...
		// Prepare rain data
		add( event, builder, WeatherDatumIdentifier.RAIN_RATE, "rainin", "0.00" );
		add( event, builder, WeatherDatumIdentifier.RAIN_TOTAL_DAILY, "dailyrainin", "0.00" );
	...
	private void add( WeatherDataEvent event, StringBuilder builder, WeatherDatumIdentifier identifier, String key, String format ) {
		Double value = event.getValue( identifier );
		if( value == null ) return;
		add( builder, value, key, format );
	}
